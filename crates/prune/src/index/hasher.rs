...
pub fn hash_file_md5<P: AsRef<Path>>(path: P) -> Result<String> {
    ...
    let file = File::open(path).map_err(|_e| PruneError::FileNotFound(path.to_path_buf()))?;
...
pub fn hash_file_blake3<P: AsRef<Path>>(path: P) -> Result<String> {
    let path = path.as_ref();
    let file = File::open(path).map_err(|_| PruneError::FileNotFound(path.to_path_buf()))?;
    let mut reader = BufReader::with_capacity(BUFFER_SIZE, file);
...
mod tests {
    ...
    fn test_verify_hash_mismatch() {
        ...
        assert!(matches!(result.unwrap_err(), PruneError::HashMismatch { .. }));
    }
