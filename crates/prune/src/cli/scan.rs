...
use console::style;
use prune_lib::{Database, SqliteDatabase, scan_directory, ScanOptions, db::drives::mark_drive_scanned};
...
fn scan_single_drive(
    ...
    let drive = db.get_drive(label)?
        .ok_or_else(|| PruneError::DriveNotFound(label.to_string()))?;

    if !drive.is_online {
        return Err(PruneError::DriveOffline(label.to_string()));
    }

    let mount_path = drive.mount_path
        .as_ref()
        .ok_or_else(|| PruneError::Config(format!("Drive '{}' has no mount path", label)))?;
    ...
    if !scan_path.exists() {
        return Err(PruneError::FileNotFound(scan_path));
    }
