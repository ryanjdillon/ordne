...
impl ClassificationRules {
    ...
    pub fn from_toml(toml_str: &str) -> Result<Self> {
        let mut config: toml::Value = toml::from_str(toml_str)
            .map_err(|e| PruneError::Config(format!("Failed to parse TOML: {}", e)))?;
        ...
        if let Some(rules_table) = config.get_mut("rules").and_then(|v| v.as_table_mut()) {
            for (name, rule_value) in rules_table.iter_mut() {
                let mut rule: ClassificationRule = rule_value.clone().try_into()
                    .map_err(|e| PruneError::Config(format!("Failed to parse rule '{}': {}", name, e)))?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        let toml_string = toml::to_string_pretty(&config)
            .map_err(|e| PruneError::Config(format!("Failed to serialize rules: {}", e)))?;
            ...
            if let RuleType::Pattern { patterns } = &rule.rule_type {
                ...
                for pattern in patterns {
                    let glob = Glob::new(pattern)
                        .map_err(|e| PruneError::Config(format!("Invalid glob pattern '{}': {}", pattern, e)))?;
                    builder.add(glob);
                }
                let globset = builder.build()
                    .map_err(|e| PruneError::Config(format!("Failed to build globset: {}", e)))?;
