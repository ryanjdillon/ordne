...
pub fn execute_rsync<S: AsRef<Path>, D: AsRef<Path>>(
    ...
    if !source.exists() {
        return Err(PruneError::FileNotFound(source.to_path_buf()));
    }
    ...
    cmd.arg(source.to_str().ok_or_else(|| {
        PruneError::Migration("Invalid source path encoding".to_string())
    })?);
    cmd.arg(dest.to_str().ok_or_else(|| {
        PruneError::Migration("Invalid destination path encoding".to_string())
    })?);
    ...
    let output = cmd.output().map_err(|e| PruneError::ExternalTool {
