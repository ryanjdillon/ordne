...
The Migration Module is the most critical component of Prune, responsible for safely executing file operations with comprehensive safety guarantees. This module prioritizes data safety above all else.
...
- `PruneError::HashMismatch`: Hash verification failed
- `PruneError::InsufficientSpace`: Not enough space
- `PruneError::SourceChanged`: Source file modified during migration
- `PruneError::DestinationVerification`: Destination verification failed
- `PruneError::DriveOffline`: Required drive not available
- `PruneError::ExternalTool`: rsync/rclone failure
