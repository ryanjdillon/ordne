...
echo "=== Ordne Phase 1 Foundation Verification ==="
echo
...
if [[ -f "crates/ordne/Cargo.toml" ]] && [[ -f "crates/ordne/src/lib.rs" ]] && [[ -f "crates/ordne/src/main.rs" ]]; then
    echo "   ✓ Ordne crate structure correct"
else
    echo "   ✗ Ordne crate structure incomplete"
    exit 1
fi
...
required_files=(
    "crates/ordne/src/error.rs"
    "crates/ordne/src/config.rs"
    "crates/ordne/src/db/mod.rs"
    "crates/ordne/src/db/schema.rs"
    "tests/common/mod.rs"
)
...
for table in "${required_tables[@]}"; do
    if grep -q "CREATE TABLE.*${table}" crates/ordne/src/db/schema.rs; then
        echo "   ✓ Table: $table"
    else
...
if grep -q "CREATE INDEX" crates/ordne/src/db/schema.rs; then
    index_count=$(grep -c "CREATE INDEX" crates/ordne/src/db/schema.rs || true)
